...
Source(시작점)에서 Sink(도착점)으로  얼마나 많은 유량을 동시에 보낼 수 있는지 계산하는 문제에서 해답을 구하는 것을 도와준다. 시작점에서 도착점까지의 최대 유량을 구할떄 주어진 정점을 탐색하는 경우 DFS를 써서 탐색한다.
...
식으로 간단히 표현하면 C(A, B)을 정점A  에서 B로 보낼수 있는 용량 값이라하고 F(A, B)을 정점 A 에서 B로 쓴 유량 값이라 한다면
F(A, B)<= C(A, B)이 성립한다.
