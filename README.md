...
간선 A-B  가 있고, A-B의 capacity가 8이 라면, B-A 라는 가상의 간선이 있고, 해당 간선의 용량은 0 이라고 가정하는 것. 이때 해당 간선의 용량이 0이므로 논리값에 큰 영향을 주지는않는다. 이때 간선의 유량 F(A,B)=-F(B,A)  공식이 성립한다.
...
최대 유량이 2N임을 DFS 탐색을 통해서는 연산과정의 횟수가 최대유량에 비례하는 반면 BFS를   두번의 과정을 통해 구해낼수 있다.
